...
CREATE TABLE IF NOT EXISTS clearing.accounting_event_type (
...
CREATE TABLE IF NOT EXISTS clearing.accounting_rule_header (
    rule_header_id SERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS clearing.accounting_rule_line (
    rule_line_id SERIAL PRIMARY KEY,
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT ck_rule_line_dr_cr CHECK (dr_cr_flag IN ('DR','CR')),
    CONSTRAINT ck_amount_source CHECK (amount_source IN ('EVENT_TOTAL','PER_ALLOCATION'))
);

CREATE TABLE IF NOT EXISTS clearing.accounting_event (
    ...
    UNIQUE (event_type_id, request_id, idempotency_key)
);

CREATE TABLE IF NOT EXISTS clearing.voucher_header (
...
VALUES ('EMP_REQ_SETTLED', 'Employee request fully allocated / settled')
